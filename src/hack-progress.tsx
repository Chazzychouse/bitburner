...
function Timer({ ns }: { ns: NS }) {
    ...
    if (progress && progress.finishedAt > progress.startedAt) {
        ...
        countdown = <span style={{fontWeight: 'bold', marginLeft: 10, color: '#333'}}>{formatTime(remaining)}</span>;
        progressBar = (
            <div style={{
                ...
                <div style={{
                    background: 'repeating-linear-gradient(135deg, #4caf50 0 16px, #43a047 16px 32px)',
                    width: `${percent * 100}%`,
                    height: '100%',
                    borderRadius: 8,
